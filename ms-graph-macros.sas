...
%macro get_access_token(auth_code, debug=0);
   ...
   filename hdrs "%sysfunc(pathname(work, L))/headers.txt";

   proc http
   /*url="&msloginBase./&tenant_id./oauth2/token"*/
      ...
      */
      in=%sysfunc(quote(%qsysfunc(urlencode(%nrstr(&client_id)=&client_id.%nrstr(&code)=&auth_code.%nrstr(&redirect_uri)=&redirect_uri%nrstr(&grant_type)=authorization_code%nrstr(&resource)=&resource.)), %qsubstr('',1,1)))
         ...
         */
         ...
         %if %sysfunc(exist(attrs.root)) %then %do;
            data _null_;
               ...
               filesize = size;
               msg = cat(name,' uploaded. File size: ',put(size,comma15. -L),' bytes. Created '
                        ,put(createdDate,datetime20.), ', Modified ',put(modifiedDate,datetime20.));
               put 'NOTE: ' msg;
            run;
         %end;
         libname attrs clear;
